...
/**
      ...
      * @brief Appends the given file to the files_ vector using move_semantics on the parameter File object, if a file with the same name does not exist within the files_ vector
      ...
      bool Folder::addFile(File& new_file) {
         //If the file' name is empty, do nothing
         if (new_file.getName().empty()) { return false; } 
         //Check if a file with the same name already exists in the files_ vector 
         for (const auto& file : files_) {
            if (new_file.getName() == file.getName()) { return false; }
         }
         //Add the file to the files_ vector
       ...
       */
      bool Folder::moveFileTo(const std::string &name, Folder &destiantion) {
         for(const auto& file: destiantion.files_) {
            if(file.getName() == name) { return false; }
         }
         for(auto it = files_.begin(); it != files_.end(); ++it) {
            if(it->getName() == name) {
               destiantion.files_.push_back(std::move(*it));
               it = files_.erase(it);
               return true;
            }
         }
      ...
      /**
         ...
         */
      bool Folder::copyFileTo(const std::string &name, Folder &destiantion){ 
         for(auto it = files_.begin(); it != files_.end(); ++it) {
            if(it->getName() == name) {
               for(const auto& file: destiantion.files_) {
                  if(file.getName() == name) { return false; }
               }
               destiantion.files_.push_back(*it);
               return true;
            }
         }
         return false;
